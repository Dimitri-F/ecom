<?php
?>

<!-- component -->
<div class="h-screen bg-gray-50 flex items-center">
    <section class="w-full bg-cover bg-center py-32" style="background-image: url('Public/assets/images/jumbotron.jpg');">
        <div class="container mx-auto text-center ">
            <h1 class="text-5xl font-medium mb-6">Bienvenue dans mon agence</h1>
            <p class="text-xl mb-12">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nam viverra euismod odio,
                gravida pellentesque urna varius vitae.</p>
        </div>
    </section>
</div>
